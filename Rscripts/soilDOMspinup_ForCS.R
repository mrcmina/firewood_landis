...
setwd("C:/.../FireWood-LANDIS/inputs_all/")  #working directory. Please adjust accordingly. 
simpath <- "C:/.../outputs/"                 #path with simulation output folder for the desired landscape (if different from wd)
...
log_Pools0 <- log_Pools %>% filter(Time == 0) %>% select(-X) #keep only Year 0
...
log_Pools0_eco <- log_Pools0_eco %>% select(ecoregion_sim, species, DOMpool, Amount)
log_Pools0_eco$Amount <- round(log_Pools0_eco$Amount, 4)
log_Pools0_eco$Amount[log_Pools0_eco$Amount == 0] <- 0.0001 #If no soil spinup is run, values cannot be 0. Thus, replace 0 with 0.0001 (mostly for Pool10)
...
## and add missing species with dummy minimum value 0.001 (otherwise the pool cannot be initialized and model does not start)
## This issue should not occur if an initial community file with maximum ages is used for DOMsoil spinup process.
...
for(e in 1:length(ecoregions)) {
  ...
  speciesadd <- setdiff(speciesnames,specieseco) #species absent in ecoregion
...
EcoSppDOMParam <- select(EcoSppDOMParam,ecoregion_sim,species, DOMpool, DecayRate, Amount, Q10)
